...
Atom right_angle_new(Atom x3, Atom z_axis, Atom np_axis, Atom part, Atom rot_axis, double angle)
{
    double check_angle = abs(acos( x3.dot(z_axis) ));
    Atom x4=np_axis;
    x4.rotate(rot_axis,-angle);

    double check_angle2= abs(acos(x4.dot(z_axis)));

    if(check_angle < check_angle2)
    {
        part.rotate(rot_axis, angle);
    }
    else{
...
Atom com_N(rvec* frame, Data& data, vector<int> mol_tag, vector<int> Nneigh)
{
    Atom cm;
    int count=0;
    for(int i=0; i<data.temp_beads.size(); ++i)
    {
        for(int& mtag : mol_tag)
        {
            if(data.temp_beads[i].mol_tag == mtag && Nneigh[i]>3)
            {
                cm.x += frame[i][0];
                cm.y += frame[i][1];
                cm.z += frame[i][2];
...
Atom axis_nano(Data& data, rvec* frame, vector <int> Nneigh)
{
    //Atom cm_ves = com(frame, data, vector<int>{1,3});
        ...
        for(int i=0; i<data.temp_beads.size(); ++i)
        {
            dist=0;
                ...
                if(data.temp_beads[i].mol_tag == 1 || data.temp_beads[i].mol_tag == 3) //&& data.temp_beads[i].type ==5 && Nneigh[i] >3)
                {
                    test.x += frame[i][0];
                    test.y += frame[i][1];
                    test.z += frame[i][2];
                ...
                if (test.z-cm_np.z < 5){
                    dist=(test.x-cm_np.x)+(test.y-cm_np.y);
                    if (dist>max_dist){
                        max_dist=dist;
                        selection=test;
                    }
        ...
        for(int i=0; i<data.temp_beads.size(); ++i)
        {
                ...
                if( data.temp_beads[i].type == 9 ) //&& data.temp_beads[i].type ==5 && Nneigh[i] >3)
                {
                    cm1.x += frame[i][0];
                    cm1.y += frame[i][1];
                    cm1.z += frame[i][2];
        ...
        for(int j=0; j<data.temp_beads.size(); ++j)
        {
                ...
                if((data.temp_beads[j].mol_tag == 1 || data.temp_beads[j].mol_tag ==3) && Nneigh[j] >3)
                {
                    cm_ves.x += frame[j][0];
                    cm_ves.y += frame[j][1];
                    cm_ves.z += frame[j][2];
...
Atom cm1_cm2_axis(Data& data, rvec* frame, vector <int> Nneigh)
{
        Atom cm1;
        int count=0;
        for(int i=0; i<data.temp_beads.size(); ++i)
        {
                ...
                if((data.temp_beads[i].mol_tag == 1  || data.temp_beads[i].mol_tag ==3 )&& Nneigh[i]>3) //&& data.temp_beads[i].type ==5 && Nneigh[i] >3)
                {
                    cm1.x += frame[i][0];
                    cm1.y += frame[i][1];
                    cm1.z += frame[i][2];
        ...
        for(int j=0; j<data.temp_beads.size(); ++j)
        {
                ...
                if(data.temp_beads[j].mol_tag == 3 && data.temp_beads[j].type == 5 && Nneigh[j] >3)
                {
                    cm2.x += frame[j][0];
                    cm2.y += frame[j][1];
                    cm2.z += frame[j][2];
...
public:
    ...
    void frame_histo(Data& data, rvec* frame, vector <int> Nneigh)
    {
        ...
        for(int j=0; j<data.temp_beads.size(); ++j) // ~10 000
        {
            ...
            {
                if ((particle.type == 2 || particle.type == 5) && (part.z  + 60) >= 0.6*b && (part.z + 60) < 0.6*(b+1))
                {
...
public:
    ...
    void grid_stalk_analysis(Data& data, rvec* frame, vector <int> Nneigh, double &min_x, double &min_y, double &z_size )
    {
        Atom cm = com_N(frame, data, vector<int>{1,3}, Nneigh);

        //double cutoff = get_cutoff(data, cm);
        //Atom axis = get_axis(data, frame, cutoff, Nneigh); // particle form the tip that fulfills symmetry
        Atom axis = cm1_cm2_axis(data, frame, Nneigh); // particle form the tip that fulfills symmetry
        Atom z_axis = Atom(0,0,1);
        axis.normalise();  // normalise axis vector for correct rotation
        Atom rot_axis = axis.cross(z_axis);
        rot_axis.normalise();                                  // normalise for rotation algo
        ...
        //rotate to align np to x axis
        //Atom cm_np = com(frame, data, vector<int>{2});
        //double cutoff = get_cutoff(data, cm);
        //Atom stalk = get_axis(data, frame, cutoff, Nneigh); // particle form the tip that fulfills symmetry
        Atom x_axis = Atom(0,1,0);
        Atom np_axis = axis_nano(data,frame, Nneigh); //cm-cm_np;                          // Axis between com and selected particle (id)
        np_axis.normalise();  // normalise axis vector for correct rotation
        Atom xrot_axis=Atom(0,0,1);

        //xrot_axis.normalise();                                  // normalise for rotation algo
        //Atom particle;
        double xangle = acos( np_axis.dot(x_axis) );
        Atom x4=np_axis;
        x4.rotate(xrot_axis, xangle);
        ...
        Atom cm_rot2=right_angle(x4, x_axis, cm_rot, xrot_axis, xangle);
        ...
        double mindist_z=999999;
        for(int j=0; j<data.temp_beads.size(); ++j) // ~10 000
        {
            particle.x = frame[j][0]-cm.x;
            particle.y = frame[j][1]-cm.y;
            particle.z = frame[j][2]-cm.z;
            particle.type = data.temp_beads[j].type;
            particle.mol_tag=data.temp_beads[j].mol_tag;

            // determine whether you want +angle or - angle and rotate
            //Atom part0=right_angle(x3, z_axis, particle, rot_axis, angle);
            Atom part=right_angle_new(x4, x_axis, np_axis, particle, xrot_axis, xangle);

            // center system on the grid
            //part.x=part.x+(offset-cm_rot2.x);
            //part.y=part.y+(offset-cm_rot2.y);

            if ( (particle.mol_tag == 1 || particle.mol_tag == 3) && (part.z -cm.z) < 15 && (part.z - cm.z > -15)){

                build_grid(part,cm,grid);
            }

            // Analysis of z size of stalk, smallest distance between inner leaflets if each vesicle
            Atom check;
            if (particle.type == 6 && particle.mol_tag ==1){

                for(int k=0; k<data.temp_beads.size(); ++k) // ~10 000
                {
                    check.x = frame[k][0]-cm.x;
                    check.y = frame[k][1]-cm.y;
                    check.z = frame[k][2]-cm.z;
                    check.type = data.temp_beads[k].type;
                    check.mol_tag=data.temp_beads[k].mol_tag;

                     if (check.type == 6 && check.mol_tag ==3){
                         double distance=particle.dist(check);
                         if(distance<mindist_z){
                             mindist_z=distance;
                         }
                     }
        ...
        //else {
         //   cout <<step<<" 0 ";
        //}
        print_grid(grid, min_x, min_y);
        //exit(1);
    }

    void grid_analysis(Data& data, rvec* frame, vector <int> Nneigh, int& pore)
    {
        Atom cm = com(frame, data, vector<int>{1,3});
        double cutoff = get_cutoff(data, cm);
        Atom id = get_axis(data, frame, cutoff, Nneigh);
        ...
        //
        //if( is_pore(start_rot, grid,start2_rot) >= 1 )
        pore=is_pore(start_rot, grid,start2_rot);

            cout << step << " " << pore; //is_pore(start_rot, grid,start2_rot); // 0 - no pore, 1 - leaky pore, 2 - pore between vesicles
    ...
    int analyze_histogram(string inName, Data& data, int stop=-1)
    {
        ...
        if (status == exdrOK)
        {
            ...
            if (xfp != NULL)
            {
                ...
                int pore;
                ofstream stalk_file;
                stalk_file.open ("stalk_analysis");
                ...
                while(status == exdrOK && ( stop == -1 || step < first_step+stop) ) // Analyze frame
                {
                    ...
                    grid_analysis( data, frame, Nneigh, pore );
                    ...
                    clusterize(data, cluster, Nneigh, neigh_index, init_stalk, vector<int>{2,3,5,6});
                    ...
                    // stalk dimension analysis
                    if (stalk ==1 && pore == 0){
                        double min_x=999;
                        double min_y=999;
                        double z_size;
                        grid_stalk_analysis( data, frame, Nneigh , min_x, min_y, z_size);
                        stalk_file<<n_frames<<" "<<min_x*0.25<<" "<<min_y*0.25<<" "<<z_size<<endl;
                        total_min_x=total_min_x+min_x;
                        total_min_y=total_min_y+min_y;
                        total_z=total_z+z_size;
                        n_frames=n_frames+1;
                    }
                ...
                stalk_file<<"#avg min_x: "<<total_min_x/n_frames<<endl;
                stalk_file<<"#avg min_y: "<<total_min_y/n_frames<<endl;
                stalk_file<<"#avg z: "<<total_z/n_frames<<endl;
                stalk_file.close();
    ...
    void print_grid(vector<vector<vector< tuple<int,bool> >>>& grid, double &min_x, double &min_y)
    {
        ...
        for (int z=0;z<grid_size;++z)
        {
            for (int y=0;y<grid_size;++y)
            {
                ...
                {
                    if (get<0>(grid[x][y][z])>0 && count_pos==0)
                    {   
        ...
        for (int z=0;z<grid_size;++z)
        {
            for (int x=0;x<grid_size;++x)
            {
                ...
                for (int y=0;y<grid_size;++y)
                {
                    if (get<0>(grid[x][y][z])>0 && count_posy==0)
                    {
